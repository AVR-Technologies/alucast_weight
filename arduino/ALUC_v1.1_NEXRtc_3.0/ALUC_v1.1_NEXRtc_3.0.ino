...
void addData() {
  ...
  EEPROM.update(locations[9], length + (config[8].length() + 2));
  //$logSerial.print(9);
  //$logSerial.print(F(" mem="));
  for (int j = 0; j < length; j++) {
    EEPROM.update(locations[9] + 1 + j, data[j]);
    //$logSerial.print(data[9][j]);
  }
  data = config[8];
  for (int j = length; j < (length + data.length()); j++) {
    EEPROM.update(locations[9] + 1 + j, data[j - length]);
    //$logSerial.print(data[9][j-length]);
  }
  EEPROM.update(locations[9] + (length + data.length() + 1), ':');
  //$logSerial.println();
  data = "";
}
...
void readDatbase() {
  ...
  for (int i = 1; i < 10; i++) {
    ...
    data = "";
    for (int j = 0; j < length; j++) {
      data += (char)EEPROM.read(locations[i] + 1 + j);
    }
      EEPROM.update(locations[i-1], length);
      ...
      nex.print(F(" mem="));
      for (int j = 0; j < length; j++) {
        EEPROM.update(locations[i-1] + 1 + j, data[j]);
        nex.print(data[j]);
      }
      nex.println();
  }
