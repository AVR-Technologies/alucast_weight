...
//        readFromEeprom(Reports_page);
        ...
        for (int i = 0; i < 10; i++){ config[i]=""; data[i] = ""; if(i<4) Weight[i] = "";}
        delay(500);
        //$logSerial.println("page " + String(pageId) + " Reports");
        sendCommand("page 7");
//        Serial.flush();
...
//  //$logSerial.println(data[9]);
  int length = data[9].length();
  EEPROM.update(locations[9], length+(config[8].length()+2));
  //$logSerial.print(9);
  //$logSerial.print(F(" mem="));
  for (int j = 0; j < length; j++) {
    EEPROM.update(locations[9] + 1 + j, data[9][j]);
    //$logSerial.print(data[9][j]);
  }
  data[9]=config[8];
  for (int j = length; j < (length+data[9].length()); j++) {
    EEPROM.update(locations[9] + 1 + j, data[9][j-length]);
    //$logSerial.print(data[9][j-length]);
  }
  EEPROM.update(locations[9]+(length+data[9].length()+1), ':');
  //$logSerial.println();
  data[9]="";
}
void saveToEeprom() {
