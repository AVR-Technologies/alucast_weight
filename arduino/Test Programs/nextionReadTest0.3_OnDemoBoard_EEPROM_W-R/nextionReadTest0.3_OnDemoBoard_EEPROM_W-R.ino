...
SoftwareSerial Printer(4, 3);
SoftwareSerial Wt_scale(A4, A5);
  ...
  if (nexSerial.available()) {
    ...
    /***********Page 7****************/
      ...
      if ((byte)2 == buttonId) {
        ...
        nexSerial.print("t0.txt=\"" + (String)Reports_page + "\"" + endBytes);
        readFromEeprom(Reports_page);
      }
    }
...
void DisplayResults()
{
  ...
  nexSerial.print("p6t7.txt=\"" + config[5] + "\"" + endBytes);
  nexSerial.print("p6t9.txt=\"" + config[6] + "\"" + endBytes);
  nexSerial.print("p6t8.txt=\"" + config[7] + "\"" + endBytes);
...
void readFromEeprom(int Report_page) {
  ...
  Data= "";
  for (int j = 0; j < length; j++) {
    Data += (char)EEPROM.read(locations[Report_page] + 1 + j);
  }
  //  }
  for (int i = 0; i < 8; i++) {
    int last = Data.indexOf(':');
